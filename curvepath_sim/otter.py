...
from .gnc import Smtrx, Hmtrx, Rzyx, Tzyx, attitudeEuler, crossFlowDrag, m2c, sat

def attitudeEuler3dof(eta,nu,sampleTime=0.02):
    p_dot   = np.matmul( Rzyx(0, 0, 0), nu[0:3] )
    v_dot   = np.matmul( Tzyx(0, 0),    np.array([0,0]) )
...
class Otter:
    ...
    def __init__(self, 
        ...
        self.nu3dof = np.array([0, 0, 0, 0, 0, 0], float)
        self.eta3dof = np.array([0, 0, 0, 0, 0, 0], float)
        ...
        self.tau_damp = np.array([0,0,0,0,0,0])
        self.tau_crossflow = np.array([0,0,0,0,0,0])
    ...
    def dynamics(self, eta, nu, u_actual, u_control, sampleTime):
        ...
        self.tau_damp = -np.matmul(self.D, nu_r)

        self.tau_damp[5] = self.tau_damp[5] - 10 * self.D[5, 5] * abs(nu_r[5]) * nu_r[5]

        # State derivatives (with dimension)
        self.tau_crossflow = crossFlowDrag(self.L, self.B_pont, self.T, nu_r)
    ...
    def current_disturbance(self,magnetude,direction):
        self.V_c = magnetude
        self.beta_c = direction

    def dynamic_eq(self):
        self.M_inv_3 = self.Minv[[0,1,5],:][:,[0,1,5]]
        self.D_3 = self.D[[0,1,5],:][:,[0,1,5]]
        self.C_3 = self.C[[0,1,5],:][:,[0,1,5]]
        self.G_3 = self.G[[0,1,5],:][:,[0,1,5]]
        # self.tau_damp_3 = self.tau_damp
    ...
    def function(self,sampleTime=0.02):
        ...
        self.speed=np.sqrt(self.nu[0]**2+self.nu[1]**2)
    ...
    def thruster(self,thrust):
        ...
        if thrust > 0:  # positive thrust    
            n = math.sqrt(thrust/self.k_pos)
        else:  # negative thrust
            n = -1*(math.sqrt(-1*thrust/self.k_neg))

        n = sat(n, self.n_min, self.n_max)  # saturation, physical limits
            
        return n


##################################################################################################
"""sampling time 0.1'den buyuk olmamali (default=0.02)
    input degerleri
    -------------------------
    current_eta=6 DOF konumlar
    nu=6 DOF hizlar
    u_actual=pervane hizi rad/s pervane (modelinin calismasi icin)
    u_control=pervane hiz referans degeri rad/s
    ****************
    output degerleri
    -------------------------------------------
    current_eta: X-Y de hiz grafilerini cizdirebilmek icin
    heading : anlik dunya koordinatina göre heading 
    velocity: aracin gittigi u-v bileske hizi
    u-actual: pervane modelinin calismasi icin sisteme tekrar girecek pervane rpm degeri
...
if __name__ == '__main__':
    ...
    for i in range(1000):
        ...
        if i < 500:
            ...
            pervane = [75,100]
        time += 0.02 
        USV_model.u_control=pervane
        output_usv = USV_model.function()
        x_position.append(USV_model.current_eta[0])
        y_position.append(USV_model.current_eta[1])
